...
const firebaseConfig = {
    apiKey: "AIzaSyCawuDT2hPttsR0FGrTAen30BtcXsFKhx8",
    authDomain: "magica-pro.firebaseapp.com",
    databaseURL: "https://magica-pro-default-rtdb.firebaseio.com",
    projectId: "magica-pro",
    storageBucket: "magica-pro.firebasestorage.app",
    messagingSenderId: "1061711424081",
    appId: "1:1061711424081:web:8585c6efd6bb091a862670"
};
...
async function registerUser() {
    ...
    try {
        ...
        // El usuario será redirigido automáticamente por onAuthStateChanged
    } catch (error) {
...
async function checkUserAccess(user) {
    try {
        ...
        if (!userData) {
            showToast('Error: Datos de usuario no encontrados', 'error');
            auth.signOut();
            return;
        }
        ...
        if (userData.status !== 'active') {
            showToast('Su cuenta ha sido desactivada. Contacte al administrador.', 'error');
            auth.signOut();
            return;
        }
        ...
        if (user.email === 'luishparedes94@gmail.com') {
...
function showMainApp(userData) {
    ...
    const planBadge = document.getElementById('userPlanDisplay');
    planBadge.textContent = userData.plan === 'pro' ? 'PRO' : 'FREE';
    planBadge.className = userData.plan === 'pro' ? 'plan-badge plan-pro' : 'plan-badge plan-free';

    // Inicializar la aplicación de calculadora
    inicializarAplicacionCalculadora();
}

// Cargar lista de usuarios (para administrador)
        ...
        for (const userId in users) {
            if (users.hasOwnProperty(userId)) {
                ...
                row.innerHTML = `
                    ...
                    <td>${user.email}</td>
                    <td>
                        <span class="plan-badge ${user.plan === 'pro' ? 'plan-pro' : 'plan-free'}">
                    ...
                    <td>
                        <input type="number" value="${user.maxDevices || 1}" min="1" max="10" 
                               onchange="updateUserDevices('${userId}', this.value)" 
                               style="width: 60px; padding: 5px;">
                    </td>
                    <td>
                        ${user.status === 'active' ? 
                            `<button class="btn-action btn-deactivate" onclick="toggleUserStatus('${userId}', 'inactive')">Desactivar</button>` :
                            `<button class="btn-action btn-activate" onclick="toggleUserStatus('${userId}', 'active')">Activar</button>`
                        }
                        ${user.plan === 'free' ?
                            `<button class="btn-action btn-pro" onclick="upgradeToPro('${userId}')">Hacer Pro</button>` :
                            `<button class="btn-action" onclick="downgradeToFree('${userId}')">Quitar Pro</button>`
                        }
                        ${user.email !== 'luishparedes94@gmail.com' ?
                            `<button class="btn-action btn-delete" onclick="deleteUser('${userId}')">Eliminar</button>` :
                            ''
                        }
                    </td>
                `;
...
async function toggleUserStatus(userId, status) {
    try {
        ...
        showToast(`Usuario ${status === 'active' ? 'activado' : 'desactivado'} correctamente`, 'success');
        loadUsersList();
    } catch (error) {
...
async function upgradeToPro(userId) {
    try {
        ...
        showToast('Usuario actualizado a Plan Pro', 'success');
        loadUsersList();
    } catch (error) {
        console.error('Error al actualizar a Pro:', error);
...
async function downgradeToFree(userId) {
    try {
        ...
        showToast('Usuario cambiado a Plan Free', 'success');
        loadUsersList();
    } catch (error) {
        console.error('Error al cambiar a Free:', error);
        showToast('Error al cambiar el plan', 'error');
    }
}

async function updateUserDevices(userId, maxDevices) {
    try {
        await database.ref('users/' + userId).update({
            maxDevices: parseInt(maxDevices)
        });
        showToast('Límite de dispositivos actualizado', 'success');
    } catch (error) {
        console.error('Error al actualizar dispositivos:', error);
        showToast('Error al actualizar dispositivos', 'error');
    }
}
...
// ===== INICIALIZACIÓN DE LA APLICACIÓN CALCULADORA =====
function inicializarAplicacionCalculadora() {
...
// ===== FUNCIONES ORIGINALES DE LA CALCULADORA (COMPLETAS) =====

// ===== INICIALIZACIÓN =====
document.addEventListener('DOMContentLoaded', function() {
    console.log('Sistema iniciado correctamente');
    // Solo mostrar sistema de autenticación inicialmente
    showAuthSystem();
    
    // Configurar eventos de formularios de autenticación
...
// Eliminar protección contra F12 y herramientas de desarrollo para mejor compatibilidad
// Se mantiene solo la funcionalidad esencial

// Inicialización simplificada - sin redirecciones
function inicializarSistemaInactividad() {
    // Sistema de inactividad básico sin redirecciones
}

// Eliminar todas las redirecciones al portal
// Se mantiene solo la funcionalidad de la calculadora y el sistema de usuarios
